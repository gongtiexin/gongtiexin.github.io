  ...
  var sidebarToggleMotion = {
    ...
    mouseupHandler: function(event) {
      ...
      var clickingBlankPart =
        Math.sqrt(deltaX * deltaX + deltaY * deltaY) < 20 && event.target.matches('.main');
      if (
        this.isSidebarVisible &&
        (clickingBlankPart || event.target.matches('img.medium-zoom-image, .fancybox img'))
      ) {
        this.hideSidebar();
      }
    },
    ...
    touchendHandler: function(event) {
      ...
      var effectiveSliding =
        Math.abs(deltaY) < 20 && ((deltaX > 30 && isRight) || (deltaX < -30 && !isRight));
